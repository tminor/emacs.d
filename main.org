...
* Intro
:PROPERTIES:
:HASH:     -173578164381488492
:MODIFIED: 2019-03-06 10:08
:END:
...
- [[#other][Other]]
  ...
  - [[#elisp-for-various-things][Elisp for various things]]
  - [[#try][Try]]
  - [[#which-key][Which Key]]
...
=custom.el= tends to litter =init.el= with things that are hard to
track; if we set things via the =customize-= interface, =custom.el=
puts changes here.
...
* Prog-mode
:PROPERTIES:
:HASH:     11050576340372672
:MODIFIED: 2019-03-06 08:46
:END:

** Language modes
:PROPERTIES:
:HASH:     11050576340372672
:MODIFIED: 2019-03-06 13:48
:END:
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun tm/update-modification-time ()
    ...
    (org-set-property "HASH" (format "%s" (tm/getentryhash)))
  ...
  (defun tm/skip-nonmodified ()
    ...
    (let ((next-headline (save-excursion (or (outline-next-heading) (point-max)))))
      (if (string= (org-entry-get (point) "HASH" nil) (format "%s" (tm/getentryhash)))
      next-headline
        nil)))
#+END_SRC
